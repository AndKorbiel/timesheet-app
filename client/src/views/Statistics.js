  ...
  generateList = (list) => {
    ...
    list.forEach((el) => {
      el.timesheets.forEach((timesheet) => {
        if (
          filters.indexOf(this.formatDate(timesheet.selectedDate)) < 0 &&
          this.formatDate(timesheet.selectedDate).length > 1
        ) {
      ...
      for (let [innerKey, innerValue] of Object.entries(value)) {
        if (innerValue.length === 0) {
          ...
          innerValue.forEach((el) => {
            if (
              filters.indexOf(this.formatDate(el.timesheet.selectedDate)) < 0 &&
              this.formatDate(el.timesheet.selectedDate).length > 1
            ) {
            ...
            <Paper className="app-main">
              ...
              <TableContainer component={Paper}>
                <Table aria-label="timesheets list table">
                  ...
                  {this.state.tsList &&
                    Object.entries(this.state.tsList).map(([key, value]) => {
                      return (
                        <TableBody>
                                    ...
                                    <Accordion>
                                      ...
                                      >
                                        <TableRow className="title-row">
                                          <TableCell colSpan={5}>
                                            {
                                              this.props.translations.months[
                                                parseInt(innerKey - 1)
                                              ]
                                            }
                                          </TableCell>
                                          <TableCell>
                                            {this.calculateMonthTotalTime(
                                              innerKey,
                                              innerValue,
                                            )}{' '}
                                            {
                                              this.props.translations
                                                .timesheets_list_table_hours
                                            }
                                          </TableCell>
                                          <TableCell>
                                            {this.calculateMonthTotalPages(
                                              innerKey,
                                              innerValue,
                                            )}{' '}
                                            {
                                              this.props.translations
                                                .timesheets_list_table_pages
                                            }
                                          </TableCell>
                                        </TableRow>
                                      </AccordionSummary>
                                      {Object.entries(innerValue)
                                        .sort()
                                        .map(([nKey, nValue]) => {
                                          return (
                                            <Table>
                                              ...
                                              <TableRow className="inner-header">
                                                ...
                                                <TableCell>
                                                  {
                                                    this.props.translations
                                                      .timesheets_list_table_project_name_label
                                                  }
                                                </TableCell>
                                                <TableCell>
                                                  {
